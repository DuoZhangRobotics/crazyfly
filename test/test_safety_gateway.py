...
def _gateway(state: SafetyState, operator_enabled: bool):
    ...
    gateway.machine.mark_landing = lambda: setattr(gateway.machine, "state", SafetyState.LANDING)
    gateway.rejections = []
    gateway.forwards = []
    gateway._reject = lambda command, reason: gateway.rejections.append((command, reason))
    gateway._forward = lambda name, kind, request: gateway.forwards.append((name, kind)) or True
...
def test_duplicate_land_is_rejected_while_landing() -> None:
    ...
    response = SimpleNamespace()
    assert gateway._land_callback("cf1", _request(1.0), response) is response
    assert gateway.rejections == [("land/cf1", "landing is already in progress")]
    assert gateway.forwards == []
